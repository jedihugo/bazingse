...
import { BRANCHES, type BranchName } from './core';
import { DAY_OFFICERS, DAY_OFFICER_ORDER, getDayOfficer } from './derived';

// ---------------------------------------------------------------------------
// Types
// ---------------------------------------------------------------------------

export type DongGongRating = "excellent" | "auspicious" | "fair" | "inauspicious" | "dire";

export interface DongGongRatingInfo {
  readonly value: number;
  readonly symbol: string;
  readonly chinese: string;
}

export interface ThreeKillingsInfo {
...
export interface MonthOverview {
  readonly chinese: string;
  readonly monthly_virtue: string;
  readonly monthly_benevolence: string;
  readonly motherly_storage?: readonly string[];
  readonly heavenly_virtue: string;
  readonly four_extinction?: string;
  readonly four_separation?: string;
  readonly three_killings?: ThreeKillingsInfo;
  readonly notes: string;
}

export interface DayBranchEntry {
  readonly officer: string;
  readonly pillars: Readonly<Record<string, DongGongRating>>;
  readonly good_for: readonly string[];
  readonly bad_for: readonly string[];
  readonly description_chinese: string;
  readonly description_english: string;
}

export type MonthEntry = DayBranchEntry | MonthOverview;

// ---------------------------------------------------------------------------
// Rating scale
// ---------------------------------------------------------------------------

export const DONG_GONG_RATINGS: Readonly<Record<DongGongRating, DongGongRatingInfo>> = {
...
export const DONG_GONG_DAY_OFFICERS: Readonly<Record<string, { order: number; chinese: string; english: string }>> =
  Object.fromEntries(
    Array.from({ length: 12 }, (_, i) => [
      DAY_OFFICERS[i].id,
      { order: i, chinese: DAY_OFFICERS[i].chinese, english: DAY_OFFICERS[i].english },
    ])
  );

export const DONG_GONG_OFFICER_ORDER = DAY_OFFICER_ORDER;

export const DONG_GONG_BRANCH_ORDER: readonly BranchName[] =
  (Object.keys(BRANCHES) as BranchName[])
    .slice()
    .sort((a, b) => BRANCHES[a].dong_gong_index - BRANCHES[b].dong_gong_index);

const _monthNames = ["一", "二", "三", "四", "五", "六", "七", "八", "九", "十", "十一", "十二"];
export const DONG_GONG_MONTHS: Readonly<Record<number, { branch: BranchName; chinese: string; english: string }>> =
  Object.fromEntries(
    (Object.keys(BRANCHES) as BranchName[]).map((b) => {
      const m = BRANCHES[b].month;
      return [m, { branch: b, chinese: m === 1 ? "正月" : `${_monthNames[m - 1]}月`, english: `Month ${m}` }];
    })
  );

export const DONG_GONG_BRANCH_TO_MONTH: Readonly<Record<BranchName, number>> =
  Object.fromEntries(
    (Object.keys(BRANCHES) as BranchName[]).map((b) => [b, BRANCHES[b].month])
  ) as Record<BranchName, number>;

export function getDongGongOfficer(monthBranch: string, dayBranch: string): string | null {
  const officer = getDayOfficer(monthBranch, dayBranch);
...
export const DONG_GONG: Readonly<Record<number, Record<string, MonthEntry>>> = {
  // MONTH 1
  1: {
    overview: {
      chinese: "月德丙、月恩丙、母倉亥子、天德合壬。",
      monthly_virtue: "Bing",
      monthly_benevolence: "Bing",
      motherly_storage: ["Hai", "Zi"],
      heavenly_virtue: "Ren",
      four_extinction: "li_chun",
      three_killings: { direction: "North", starts_after: "yu_shui", sectors: ["Hai", "Zi", "Chou"] },
    ...
    Yin: {
      officer: "Jian",
      pillars: { "Jia-Yin": "inauspicious", "Bing-Yin": "inauspicious", "Wu-Yin": "inauspicious", "Geng-Yin": "inauspicious", "Ren-Yin": "inauspicious" },
      good_for: [],
    ...
    Mao: {
      officer: "Chu",
      pillars: { "Yi-Mao": "inauspicious", "Ding-Mao": "inauspicious", "Ji-Mao": "inauspicious", "Xin-Mao": "inauspicious", "Gui-Mao": "inauspicious" },
      good_for: [],
    ...
    Chen: {
      officer: "Man",
      pillars: { "Jia-Chen": "dire", "Bing-Chen": "inauspicious", "Wu-Chen": "dire", "Geng-Chen": "inauspicious", "Ren-Chen": "inauspicious" },
      good_for: [],
    ...
    Si: {
      officer: "Ping",
      pillars: { "Yi-Si": "inauspicious", "Ding-Si": "inauspicious", "Ji-Si": "inauspicious", "Xin-Si": "inauspicious", "Gui-Si": "inauspicious" },
      good_for: [],
    ...
    Wu: {
      officer: "Ding",
      pillars: { "Jia-Wu": "auspicious", "Bing-Wu": "excellent", "Wu-Wu": "auspicious", "Geng-Wu": "auspicious", "Ren-Wu": "auspicious" },
    ...
    Wei: {
      officer: "Zhi",
      pillars: { "Yi-Wei": "dire", "Ding-Wei": "inauspicious", "Ji-Wei": "inauspicious", "Xin-Wei": "inauspicious", "Gui-Wei": "inauspicious" },
      good_for: [],
    ...
    Shen: {
      officer: "Po",
      pillars: { "Jia-Shen": "inauspicious", "Bing-Shen": "inauspicious", "Wu-Shen": "inauspicious", "Geng-Shen": "dire", "Ren-Shen": "inauspicious" },
      good_for: [],
    ...
    You: {
      officer: "Wei",
      pillars: { "Yi-You": "inauspicious", "Ding-You": "auspicious", "Ji-You": "inauspicious", "Xin-You": "dire", "Gui-You": "inauspicious" },
    ...
    Xu: {
      officer: "Cheng",
      pillars: { "Jia-Xu": "dire", "Bing-Xu": "inauspicious", "Wu-Xu": "dire", "Geng-Xu": "inauspicious", "Ren-Xu": "dire" },
      good_for: [],
    ...
    Hai: {
      officer: "Shou",
      pillars: { "Yi-Hai": "inauspicious", "Ding-Hai": "inauspicious", "Ji-Hai": "inauspicious", "Xin-Hai": "inauspicious", "Gui-Hai": "inauspicious" },
      good_for: [],
    ...
    Zi: {
      officer: "Kai",
      pillars: { "Jia-Zi": "inauspicious", "Bing-Zi": "auspicious", "Wu-Zi": "auspicious", "Geng-Zi": "auspicious", "Ren-Zi": "inauspicious" },
    ...
    Chou: {
      officer: "Bi",
      pillars: { "Yi-Chou": "inauspicious", "Ding-Chou": "inauspicious", "Ji-Chou": "inauspicious", "Xin-Chou": "inauspicious", "Gui-Chou": "inauspicious" },
      good_for: [],
  ...
  2: {
    overview: {
      ...
      monthly_virtue: "Jia",
      monthly_benevolence: "Ding",
      motherly_storage: ["Hai", "Zi"],
      heavenly_virtue: "Ji",
      four_separation: "chun_fen",
      three_killings: { direction: "West", starts_after: "jing_zhi", sectors: ["Shen", "You", "Xu"] },
    ...
    Si: {
      officer: "Man",
      pillars: { "Yi-Si": "fair", "Ding-Si": "fair", "Ji-Si": "fair", "Xin-Si": "fair", "Gui-Si": "fair" },
    ...
    Wu: {
      officer: "Ping",
      pillars: { "Jia-Wu": "inauspicious", "Bing-Wu": "inauspicious", "Wu-Wu": "inauspicious", "Geng-Wu": "inauspicious", "Ren-Wu": "inauspicious" },
      good_for: ["yang_tomb"],
    ...
    Wei: {
      officer: "Ding",
      pillars: { "Yi-Wei": "dire", "Ding-Wei": "inauspicious", "Ji-Wei": "inauspicious", "Xin-Wei": "inauspicious", "Gui-Wei": "auspicious" },
    ...
    Shen: {
      officer: "Zhi",
      pillars: { "Jia-Shen": "auspicious", "Bing-Shen": "auspicious", "Wu-Shen": "auspicious", "Geng-Shen": "inauspicious", "Ren-Shen": "auspicious" },
    ...
    You: {
      officer: "Po",
      pillars: { "Yi-You": "inauspicious", "Ding-You": "inauspicious", "Ji-You": "inauspicious", "Xin-You": "dire", "Gui-You": "inauspicious" },
      good_for: [],
      bad_for: ["marriage", "renovation"],
    ...
    Xu: {
      officer: "Wei",
      pillars: { "Jia-Xu": "inauspicious", "Bing-Xu": "dire", "Wu-Xu": "inauspicious", "Geng-Xu": "inauspicious", "Ren-Xu": "dire" },
      good_for: ["yang_tomb"],
    ...
    Hai: {
      officer: "Cheng",
      pillars: { "Yi-Hai": "auspicious", "Ding-Hai": "auspicious", "Ji-Hai": "auspicious", "Xin-Hai": "excellent", "Gui-Hai": "excellent" },
    ...
    Zi: {
      officer: "Shou",
      pillars: { "Jia-Zi": "inauspicious", "Bing-Zi": "inauspicious", "Wu-Zi": "inauspicious", "Geng-Zi": "inauspicious", "Ren-Zi": "inauspicious" },
      good_for: [],
    ...
    Chou: {
      officer: "Kai",
      pillars: { "Yi-Chou": "inauspicious", "Ding-Chou": "dire", "Ji-Chou": "inauspicious", "Xin-Chou": "inauspicious", "Gui-Chou": "dire" },
      good_for: [],
  ...
  3: {
    overview: {
      ...
      monthly_virtue: "Ren",
      monthly_benevolence: "Geng",
      motherly_storage: ["Hai", "Zi"],
      heavenly_virtue: "Ding",
      three_killings: { direction: "South", starts_after: "qing_ming", sectors: ["Si", "Wu", "Wei"] },
    ...
    Si: {
      officer: "Chu",
      pillars: { "Yi-Si": "inauspicious", "Ding-Si": "excellent", "Ji-Si": "auspicious", "Xin-Si": "inauspicious", "Gui-Si": "dire" },
    ...
    Wu: {
      officer: "Man",
      pillars: { "Jia-Wu": "inauspicious", "Bing-Wu": "fair", "Wu-Wu": "inauspicious", "Geng-Wu": "dire", "Ren-Wu": "auspicious" },
    ...
    Shen: {
      officer: "Ding",
      pillars: { "Jia-Shen": "auspicious", "Bing-Shen": "auspicious", "Wu-Shen": "inauspicious", "Geng-Shen": "inauspicious", "Ren-Shen": "excellent" },
    ...
    You: {
      officer: "Zhi",
      pillars: { "Yi-You": "excellent", "Ding-You": "excellent", "Ji-You": "inauspicious", "Xin-You": "inauspicious", "Gui-You": "auspicious" },
    ...
    Hai: {
      officer: "Wei",
      pillars: { "Yi-Hai": "auspicious", "Ding-Hai": "inauspicious", "Ji-Hai": "excellent", "Xin-Hai": "inauspicious", "Gui-Hai": "dire" },
    ...
    Zi: {
      officer: "Cheng",
      pillars: { "Jia-Zi": "auspicious", "Bing-Zi": "auspicious", "Wu-Zi": "auspicious", "Geng-Zi": "auspicious", "Ren-Zi": "fair" },
    ...
    Yin: {
      officer: "Kai",
      pillars: { "Jia-Yin": "fair", "Bing-Yin": "fair", "Wu-Yin": "excellent", "Geng-Yin": "fair", "Ren-Yin": "excellent" },
  ...
  4: {
    overview: {
      ...
      monthly_virtue: "Geng",
      monthly_benevolence: "Si",
      heavenly_virtue: "Bing",
      four_extinction: "li_xia",
      three_killings: { direction: "East", starts_after: "xiao_man", sectors: ["Yin", "Mao", "Chen"] },
    ...
    Wu: {
      officer: "Chu",
      pillars: { "Jia-Wu": "excellent", "Bing-Wu": "inauspicious", "Wu-Wu": "inauspicious", "Geng-Wu": "excellent", "Ren-Wu": "excellent" },
    ...
    Wei: {
      officer: "Man",
      pillars: { "Yi-Wei": "inauspicious", "Ding-Wei": "inauspicious", "Ji-Wei": "auspicious", "Xin-Wei": "auspicious", "Gui-Wei": "inauspicious" },
    ...
    Shen: {
      officer: "Ping",
      pillars: { "Jia-Shen": "dire", "Bing-Shen": "inauspicious", "Wu-Shen": "inauspicious", "Geng-Shen": "dire", "Ren-Shen": "inauspicious" },
      good_for: [],
    ...
    You: {
      officer: "Ding",
      pillars: { "Yi-You": "inauspicious", "Ding-You": "inauspicious", "Ji-You": "inauspicious", "Xin-You": "inauspicious", "Gui-You": "inauspicious" },
      good_for: ["minor_activities"],
    ...
    Xu: {
      officer: "Zhi",
      pillars: { "Jia-Xu": "fair", "Bing-Xu": "dire", "Wu-Xu": "inauspicious", "Geng-Xu": "inauspicious", "Ren-Xu": "dire" },
      good_for: ["minor_activities"],
    ...
    Hai: {
      officer: "Po",
      pillars: { "Yi-Hai": "inauspicious", "Ding-Hai": "inauspicious", "Ji-Hai": "inauspicious", "Xin-Hai": "inauspicious", "Gui-Hai": "dire" },
      good_for: [],
      bad_for: ["all_activities"],
    ...
    Zi: {
      officer: "Wei",
      pillars: { "Jia-Zi": "inauspicious", "Bing-Zi": "auspicious", "Wu-Zi": "auspicious", "Geng-Zi": "excellent", "Ren-Zi": "inauspicious" },
    ...
    Mao: {
      officer: "Kai",
      pillars: { "Yi-Mao": "auspicious", "Ding-Mao": "auspicious", "Ji-Mao": "excellent", "Xin-Mao": "excellent", "Gui-Mao": "excellent" },
    ...
    Chen: {
      officer: "Bi",
      pillars: { "Jia-Chen": "dire", "Bing-Chen": "fair", "Wu-Chen": "dire", "Geng-Chen": "fair", "Ren-Chen": "fair" },
      good_for: ["minor_activities"],
  ...
  5: {
    overview: {
      ...
      monthly_virtue: "Bing",
      monthly_benevolence: "Wu",
      motherly_storage: ["Yin", "Mao"],
      heavenly_virtue: "Yin",
      four_separation: "xia_zhi",
      three_killings: { direction: "North", starts_after: "mang_zhong", sectors: ["Hai", "Zi", "Chou"] },
    ...
    Wu: {
      officer: "Jian",
      pillars: { "Jia-Wu": "auspicious", "Bing-Wu": "inauspicious", "Wu-Wu": "inauspicious", "Geng-Wu": "inauspicious", "Ren-Wu": "inauspicious" },
      good_for: ["burial"],
    ...
    Wei: {
      officer: "Chu",
      pillars: { "Yi-Wei": "dire", "Ding-Wei": "fair", "Ji-Wei": "fair", "Xin-Wei": "fair", "Gui-Wei": "fair" },
      good_for: ["minor_activities"],
    ...
    Shen: {
      officer: "Man",
      pillars: { "Jia-Shen": "auspicious", "Bing-Shen": "auspicious", "Wu-Shen": "auspicious", "Geng-Shen": "fair", "Ren-Shen": "inauspicious" },
    ...
    Xu: {
      officer: "Ding",
      pillars: { "Jia-Xu": "excellent", "Bing-Xu": "inauspicious", "Wu-Xu": "excellent", "Geng-Xu": "excellent", "Ren-Xu": "inauspicious" },
    ...
    Hai: {
      officer: "Zhi",
      pillars: { "Yi-Hai": "auspicious", "Ding-Hai": "fair", "Ji-Hai": "fair", "Xin-Hai": "inauspicious", "Gui-Hai": "dire" },
    ...
    Zi: {
      officer: "Po",
      pillars: { "Jia-Zi": "inauspicious", "Bing-Zi": "inauspicious", "Wu-Zi": "inauspicious", "Geng-Zi": "inauspicious", "Ren-Zi": "dire" },
      good_for: [],
    ...
    Yin: {
      officer: "Cheng",
      pillars: { "Jia-Yin": "excellent", "Bing-Yin": "excellent", "Wu-Yin": "excellent", "Geng-Yin": "excellent", "Ren-Yin": "auspicious" },
    ...
    Chen: {
      officer: "Kai",
      pillars: { "Jia-Chen": "inauspicious", "Bing-Chen": "excellent", "Wu-Chen": "inauspicious", "Geng-Chen": "excellent", "Ren-Chen": "excellent" },
    ...
    Si: {
      officer: "Bi",
      pillars: { "Yi-Si": "auspicious", "Ding-Si": "inauspicious", "Ji-Si": "inauspicious", "Xin-Si": "auspicious", "Gui-Si": "inauspicious" },
  ...
  6: {
    overview: {
      ...
      monthly_virtue: "Jia",
      monthly_benevolence: "Xin",
      motherly_storage: ["Yin", "Mao"],
      heavenly_virtue: "Ji",
      three_killings: { direction: "West", starts_after: "xiao_shu", sectors: ["Shen", "You", "Xu"] },
    ...
    Shen: {
      officer: "Chu",
      pillars: { "Jia-Shen": "excellent", "Bing-Shen": "inauspicious", "Wu-Shen": "auspicious", "Geng-Shen": "fair", "Ren-Shen": "auspicious" },
    ...
    You: {
      officer: "Man",
      pillars: { "Yi-You": "auspicious", "Ding-You": "inauspicious", "Ji-You": "inauspicious", "Xin-You": "auspicious", "Gui-You": "inauspicious" },
    ...
    Xu: {
      officer: "Ping",
      pillars: { "Jia-Xu": "auspicious", "Bing-Xu": "inauspicious", "Wu-Xu": "inauspicious", "Geng-Xu": "inauspicious", "Ren-Xu": "inauspicious" },
      good_for: [],
    ...
    Hai: {
      officer: "Ding",
      pillars: { "Yi-Hai": "excellent", "Ding-Hai": "excellent", "Ji-Hai": "auspicious", "Xin-Hai": "inauspicious", "Gui-Hai": "inauspicious" },
    ...
    Zi: {
      officer: "Zhi",
      pillars: { "Jia-Zi": "fair", "Bing-Zi": "excellent", "Wu-Zi": "auspicious", "Geng-Zi": "excellent", "Ren-Zi": "inauspicious" },
    ...
    Chou: {
      officer: "Po",
      pillars: { "Yi-Chou": "inauspicious", "Ding-Chou": "dire", "Ji-Chou": "inauspicious", "Xin-Chou": "dire", "Gui-Chou": "dire" },
      good_for: [],
    ...
    Yin: {
      officer: "Wei",
      pillars: { "Jia-Yin": "excellent", "Bing-Yin": "auspicious", "Wu-Yin": "auspicious", "Geng-Yin": "auspicious", "Ren-Yin": "auspicious" },
    ...
    Mao: {
      officer: "Cheng",
      pillars: { "Yi-Mao": "excellent", "Ding-Mao": "auspicious", "Ji-Mao": "auspicious", "Xin-Mao": "excellent", "Gui-Mao": "auspicious" },
    ...
    Chen: {
      officer: "Shou",
      pillars: { "Jia-Chen": "excellent", "Bing-Chen": "auspicious", "Wu-Chen": "inauspicious", "Geng-Chen": "inauspicious", "Ren-Chen": "auspicious" },
    ...
    Si: {
      officer: "Kai",
      pillars: { "Yi-Si": "auspicious", "Ding-Si": "inauspicious", "Ji-Si": "inauspicious", "Xin-Si": "inauspicious", "Gui-Si": "auspicious" },
    ...
    Wu: {
      officer: "Bi",
      pillars: { "Jia-Wu": "auspicious", "Bing-Wu": "fair", "Wu-Wu": "inauspicious", "Geng-Wu": "fair", "Ren-Wu": "fair" },
      good_for: ["minor_activities"],
  ...
  7: {
    overview: {
      ...
      monthly_virtue: "Ren",
      monthly_benevolence: "Ren",
      motherly_storage: ["Chen", "Xu", "Chou", "Wei"],
      heavenly_virtue: "Wu",
      four_extinction: "li_qiu",
      three_killings: { direction: "South", starts_after: "chu_shu", sectors: ["Si", "Wu", "Wei"] },
    ...
    Shen: {
      officer: "Jian",
      pillars: { "Jia-Shen": "auspicious", "Bing-Shen": "inauspicious", "Wu-Shen": "excellent", "Geng-Shen": "inauspicious", "Ren-Shen": "auspicious" },
      good_for: ["burial"],
      bad_for: ["most_activities"],
    ...
    You: {
      officer: "Chu",
      pillars: { "Yi-You": "auspicious", "Ding-You": "inauspicious", "Ji-You": "inauspicious", "Xin-You": "dire", "Gui-You": "inauspicious" },
    ...
    Xu: {
      officer: "Man",
      pillars: { "Jia-Xu": "inauspicious", "Bing-Xu": "inauspicious", "Wu-Xu": "inauspicious", "Geng-Xu": "inauspicious", "Ren-Xu": "inauspicious" },
      good_for: [],
    ...
    Zi: {
      officer: "Ding",
      pillars: { "Jia-Zi": "inauspicious", "Bing-Zi": "excellent", "Wu-Zi": "auspicious", "Geng-Zi": "auspicious", "Ren-Zi": "inauspicious" },
    ...
    Yin: {
      officer: "Po",
      pillars: { "Jia-Yin": "dire", "Bing-Yin": "inauspicious", "Wu-Yin": "inauspicious", "Geng-Yin": "inauspicious", "Ren-Yin": "auspicious" },
      good_for: ["burial"],
    ...
    Mao: {
      officer: "Wei",
      pillars: { "Yi-Mao": "dire", "Ding-Mao": "excellent", "Ji-Mao": "auspicious", "Xin-Mao": "auspicious", "Gui-Mao": "excellent" },
    ...
    Chen: {
      officer: "Cheng",
      pillars: { "Jia-Chen": "inauspicious", "Bing-Chen": "auspicious", "Wu-Chen": "inauspicious", "Geng-Chen": "auspicious", "Ren-Chen": "excellent" },
    ...
    Wu: {
      officer: "Kai",
      pillars: { "Jia-Wu": "auspicious", "Bing-Wu": "excellent", "Wu-Wu": "excellent", "Geng-Wu": "inauspicious", "Ren-Wu": "excellent" },
    ...
    Wei: {
      officer: "Bi",
      pillars: { "Yi-Wei": "inauspicious", "Ding-Wei": "auspicious", "Ji-Wei": "excellent", "Xin-Wei": "auspicious", "Gui-Wei": "excellent" },
  ...
  8: {
    overview: {
      ...
      monthly_virtue: "Geng",
      monthly_benevolence: "Gui",
      motherly_storage: ["Chen", "Xu", "Chou", "Wei"],
      heavenly_virtue: "Hai",
      four_separation: "qiu_fen",
      three_killings: { direction: "East", starts_after: "bai_lu", sectors: ["Yin", "Mao", "Chen"] },
    ...
    Xu: {
      officer: "Chu",
      pillars: { "Jia-Xu": "auspicious", "Bing-Xu": "inauspicious", "Wu-Xu": "auspicious", "Geng-Xu": "excellent", "Ren-Xu": "inauspicious" },
    ...
    Hai: {
      officer: "Man",
      pillars: { "Yi-Hai": "excellent", "Ding-Hai": "excellent", "Ji-Hai": "excellent", "Xin-Hai": "inauspicious", "Gui-Hai": "dire" },
    ...
    Chou: {
      officer: "Ding",
      pillars: { "Yi-Chou": "fair", "Ding-Chou": "fair", "Ji-Chou": "inauspicious", "Xin-Chou": "fair", "Gui-Chou": "fair" },
      good_for: ["minor_activities"],
      bad_for: [],
    ...
    Yin: {
      officer: "Zhi",
      pillars: { "Jia-Yin": "dire", "Bing-Yin": "auspicious", "Wu-Yin": "auspicious", "Geng-Yin": "excellent", "Ren-Yin": "auspicious" },
    ...
    Mao: {
      officer: "Po",
      pillars: { "Yi-Mao": "dire", "Ding-Mao": "inauspicious", "Ji-Mao": "fair", "Xin-Mao": "inauspicious", "Gui-Mao": "fair" },
      good_for: [],
      bad_for: ["important_activities"],
    ...
    Chen: {
      officer: "Wei",
      pillars: { "Jia-Chen": "inauspicious", "Bing-Chen": "auspicious", "Wu-Chen": "inauspicious", "Geng-Chen": "inauspicious", "Ren-Chen": "excellent" },
    ...
    Si: {
      officer: "Cheng",
      pillars: { "Yi-Si": "excellent", "Ding-Si": "inauspicious", "Ji-Si": "excellent", "Xin-Si": "inauspicious", "Gui-Si": "inauspicious" },
    ...
    Wu: {
      officer: "Shou",
      pillars: { "Jia-Wu": "inauspicious", "Bing-Wu": "auspicious", "Wu-Wu": "auspicious", "Geng-Wu": "inauspicious", "Ren-Wu": "excellent" },
    ...
    Wei: {
      officer: "Kai",
      pillars: { "Yi-Wei": "inauspicious", "Ding-Wei": "auspicious", "Ji-Wei": "auspicious", "Xin-Wei": "auspicious", "Gui-Wei": "auspicious" },
    ...
    Shen: {
      officer: "Bi",
      pillars: { "Jia-Shen": "auspicious", "Bing-Shen": "excellent", "Wu-Shen": "excellent", "Geng-Shen": "excellent", "Ren-Shen": "auspicious" },
  ...
  9: {
    overview: {
      ...
      motherly_storage: ["Chen", "Xu", "Chou", "Wei"],
      heavenly_virtue: "Xin",
      three_killings: { direction: "North", starts_after: "han_lu", sectors: ["Hai", "Zi", "Chou"] },
    ...
    Xu: {
      officer: "Jian",
      pillars: { "Jia-Xu": "inauspicious", "Bing-Xu": "excellent", "Wu-Xu": "inauspicious", "Geng-Xu": "inauspicious", "Ren-Xu": "inauspicious" },
    ...
    Hai: {
      officer: "Chu",
      pillars: { "Yi-Hai": "excellent", "Ding-Hai": "excellent", "Ji-Hai": "auspicious", "Xin-Hai": "dire", "Gui-Hai": "dire" },
    ...
    Zi: {
      officer: "Man",
      pillars: { "Jia-Zi": "auspicious", "Bing-Zi": "excellent", "Wu-Zi": "inauspicious", "Geng-Zi": "inauspicious", "Ren-Zi": "inauspicious" },
    ...
    Chou: {
      officer: "Ping",
      pillars: { "Yi-Chou": "inauspicious", "Ding-Chou": "excellent", "Ji-Chou": "inauspicious", "Xin-Chou": "inauspicious", "Gui-Chou": "dire" },
      good_for: ["minor_activities"],
      bad_for: ["major_activities"],
    ...
    Yin: {
      officer: "Ding",
      pillars: { "Jia-Yin": "inauspicious", "Bing-Yin": "excellent", "Wu-Yin": "excellent", "Geng-Yin": "excellent", "Ren-Yin": "inauspicious" },
    ...
    Mao: {
      officer: "Zhi",
      pillars: { "Yi-Mao": "dire", "Ding-Mao": "auspicious", "Ji-Mao": "excellent", "Xin-Mao": "excellent", "Gui-Mao": "auspicious" },
    ...
    Chen: {
      officer: "Po",
      pillars: { "Jia-Chen": "dire", "Bing-Chen": "fair", "Wu-Chen": "dire", "Geng-Chen": "fair", "Ren-Chen": "inauspicious" },
    ...
    Si: {
      officer: "Wei",
      pillars: { "Yi-Si": "excellent", "Ding-Si": "fair", "Ji-Si": "fair", "Xin-Si": "fair", "Gui-Si": "fair" },
    ...
    Wei: {
      officer: "Shou",
      pillars: { "Yi-Wei": "inauspicious", "Ding-Wei": "inauspicious", "Ji-Wei": "excellent", "Xin-Wei": "auspicious", "Gui-Wei": "auspicious" },
    ...
    Shen: {
      officer: "Kai",
      pillars: { "Jia-Shen": "excellent", "Bing-Shen": "auspicious", "Wu-Shen": "excellent", "Geng-Shen": "inauspicious", "Ren-Shen": "auspicious" },
    ...
    You: {
      officer: "Bi",
      pillars: { "Yi-You": "inauspicious", "Ding-You": "fair", "Ji-You": "inauspicious", "Xin-You": "inauspicious", "Gui-You": "inauspicious" },
  ...
  10: {
    overview: {
      ...
      monthly_virtue: "Jia",
      monthly_benevolence: "Yi",
      motherly_storage: ["Shen", "You"],
      heavenly_virtue: "Geng",
      four_extinction: "li_dong",
      three_killings: { direction: "West", starts_after: "xiao_xue", sectors: ["Shen", "You", "Xu"] },
    ...
    Hai: {
      officer: "Jian",
      pillars: { "Yi-Hai": "fair", "Ding-Hai": "inauspicious", "Ji-Hai": "fair", "Xin-Hai": "inauspicious", "Gui-Hai": "inauspicious" },
      good_for: ["minor_activities"],
    ...
    Zi: {
      officer: "Chu",
      pillars: { "Jia-Zi": "auspicious", "Bing-Zi": "inauspicious", "Wu-Zi": "inauspicious", "Geng-Zi": "inauspicious", "Ren-Zi": "inauspicious" },
      good_for: [],
      bad_for: ["most_activities"],
    ...
    Yin: {
      officer: "Ping",
      pillars: { "Jia-Yin": "excellent", "Bing-Yin": "inauspicious", "Wu-Yin": "inauspicious", "Geng-Yin": "auspicious", "Ren-Yin": "auspicious" },
      good_for: ["minor_repairs"],
      bad_for: ["major_activities"],
    ...
    Mao: {
      officer: "Ding",
      pillars: { "Yi-Mao": "auspicious", "Ding-Mao": "auspicious", "Ji-Mao": "excellent", "Xin-Mao": "excellent", "Gui-Mao": "auspicious" },
    ...
    Chen: {
      officer: "Zhi",
      pillars: { "Jia-Chen": "excellent", "Bing-Chen": "auspicious", "Wu-Chen": "dire", "Geng-Chen": "inauspicious", "Ren-Chen": "inauspicious" },
      good_for: ["minor_repairs"],
    ...
    Si: {
      officer: "Po",
      pillars: { "Yi-Si": "auspicious", "Ding-Si": "dire", "Ji-Si": "inauspicious", "Xin-Si": "inauspicious", "Gui-Si": "inauspicious" },
      good_for: ["minor_activities"],
      bad_for: ["important_activities"],
    ...
    Wu: {
      officer: "Wei",
      pillars: { "Jia-Wu": "excellent", "Bing-Wu": "dire", "Wu-Wu": "auspicious", "Geng-Wu": "auspicious", "Ren-Wu": "auspicious" },
    ...
    Wei: {
      officer: "Cheng",
      pillars: { "Yi-Wei": "inauspicious", "Ding-Wei": "auspicious", "Ji-Wei": "auspicious", "Xin-Wei": "auspicious", "Gui-Wei": "excellent" },
    ...
    Shen: {
      officer: "Shou",
      pillars: { "Jia-Shen": "excellent", "Bing-Shen": "fair", "Wu-Shen": "fair", "Geng-Shen": "dire", "Ren-Shen": "fair" },
    ...
    You: {
      officer: "Kai",
      pillars: { "Yi-You": "excellent", "Ding-You": "auspicious", "Ji-You": "inauspicious", "Xin-You": "auspicious", "Gui-You": "auspicious" },
    ...
    Xu: {
      officer: "Bi",
      pillars: { "Jia-Xu": "excellent", "Bing-Xu": "dire", "Wu-Xu": "dire", "Geng-Xu": "inauspicious", "Ren-Xu": "inauspicious" },
  ...
  11: {
    overview: {
      ...
      monthly_virtue: "Ren",
      monthly_benevolence: "Jia",
      motherly_storage: ["Shen", "You"],
      heavenly_virtue: "Shen",
      four_separation: "dong_zhi",
      three_killings: { direction: "South", starts_after: "da_xue", sectors: ["Si", "Wu", "Wei"] },
    ...
    Chou: {
      officer: "Chu",
      pillars: { "Yi-Chou": "excellent", "Ding-Chou": "auspicious", "Ji-Chou": "auspicious", "Xin-Chou": "auspicious", "Gui-Chou": "auspicious" },
    ...
    Yin: {
      officer: "Man",
      pillars: { "Jia-Yin": "fair", "Bing-Yin": "auspicious", "Wu-Yin": "excellent", "Geng-Yin": "auspicious", "Ren-Yin": "excellent" },
    ...
    Mao: {
      officer: "Ping",
      pillars: { "Yi-Mao": "auspicious", "Ding-Mao": "inauspicious", "Ji-Mao": "inauspicious", "Xin-Mao": "inauspicious", "Gui-Mao": "inauspicious" },
      good_for: [],
    ...
    Chen: {
      officer: "Ding",
      pillars: { "Jia-Chen": "inauspicious", "Bing-Chen": "inauspicious", "Wu-Chen": "inauspicious", "Geng-Chen": "inauspicious", "Ren-Chen": "auspicious" },
    ...
    Si: {
      officer: "Zhi",
      pillars: { "Yi-Si": "excellent", "Ding-Si": "dire", "Ji-Si": "excellent", "Xin-Si": "auspicious", "Gui-Si": "excellent" },
    ...
    Wu: {
      officer: "Po",
      pillars: { "Jia-Wu": "inauspicious", "Bing-Wu": "inauspicious", "Wu-Wu": "inauspicious", "Geng-Wu": "inauspicious", "Ren-Wu": "fair" },
      good_for: [],
    ...
    Wei: {
      officer: "Wei",
      pillars: { "Yi-Wei": "dire", "Ding-Wei": "excellent", "Ji-Wei": "auspicious", "Xin-Wei": "inauspicious", "Gui-Wei": "inauspicious" },
    ...
    Shen: {
      officer: "Cheng",
      pillars: { "Jia-Shen": "excellent", "Bing-Shen": "inauspicious", "Wu-Shen": "excellent", "Geng-Shen": "fair", "Ren-Shen": "excellent" },
    ...
    Xu: {
      officer: "Kai",
      pillars: { "Jia-Xu": "auspicious", "Bing-Xu": "dire", "Wu-Xu": "inauspicious", "Geng-Xu": "inauspicious", "Ren-Xu": "dire" },
      good_for: ["burial", "minor_activities"],
      bad_for: ["most_activities"],
    ...
    Hai: {
      officer: "Bi",
      pillars: { "Yi-Hai": "excellent", "Ding-Hai": "auspicious", "Ji-Hai": "excellent", "Xin-Hai": "dire", "Gui-Hai": "dire" },
  ...
  12: {
    overview: {
      ...
      motherly_storage: ["Shen", "You"],
      heavenly_virtue: "Yi",
      three_killings: { direction: "East", starts_after: "xiao_han", sectors: ["Yin", "Mao", "Chen"] },
    ...
    Chou: {
      officer: "Jian",
      pillars: { "Yi-Chou": "excellent", "Ding-Chou": "dire", "Ji-Chou": "excellent", "Xin-Chou": "fair", "Gui-Chou": "inauspicious" },
    ...
    Yin: {
      officer: "Chu",
      pillars: { "Jia-Yin": "excellent", "Bing-Yin": "excellent", "Wu-Yin": "auspicious", "Geng-Yin": "excellent", "Ren-Yin": "excellent" },
    ...
    Mao: {
      officer: "Man",
      pillars: { "Yi-Mao": "inauspicious", "Ding-Mao": "inauspicious", "Ji-Mao": "inauspicious", "Xin-Mao": "auspicious", "Gui-Mao": "inauspicious" },
    ...
    Chen: {
      officer: "Ping",
      pillars: { "Jia-Chen": "inauspicious", "Bing-Chen": "inauspicious", "Wu-Chen": "inauspicious", "Geng-Chen": "excellent", "Ren-Chen": "auspicious" },
    ...
    Si: {
      officer: "Ding",
      pillars: { "Yi-Si": "inauspicious", "Ding-Si": "dire", "Ji-Si": "inauspicious", "Xin-Si": "inauspicious", "Gui-Si": "auspicious" },
    ...
    Wu: {
      officer: "Zhi",
      pillars: { "Jia-Wu": "auspicious", "Bing-Wu": "dire", "Wu-Wu": "auspicious", "Geng-Wu": "excellent", "Ren-Wu": "excellent" },
    ...
    Wei: {
      officer: "Po",
      pillars: { "Yi-Wei": "inauspicious", "Ding-Wei": "excellent", "Ji-Wei": "inauspicious", "Xin-Wei": "inauspicious", "Gui-Wei": "excellent" },
    ...
    Shen: {
      officer: "Wei",
      pillars: { "Jia-Shen": "fair", "Bing-Shen": "fair", "Wu-Shen": "inauspicious", "Geng-Shen": "fair", "Ren-Shen": "fair" },
    ...
    You: {
      officer: "Cheng",
      pillars: { "Yi-You": "excellent", "Ding-You": "fair", "Ji-You": "excellent", "Xin-You": "auspicious", "Gui-You": "excellent" },
    ...
    Xu: {
      officer: "Shou",
      pillars: { "Jia-Xu": "auspicious", "Bing-Xu": "inauspicious", "Wu-Xu": "inauspicious", "Geng-Xu": "auspicious", "Ren-Xu": "inauspicious" },
      good_for: ["minor_activities"],
      bad_for: ["most_activities"],
    ...
    Hai: {
      officer: "Kai",
      pillars: { "Yi-Hai": "excellent", "Ding-Hai": "auspicious", "Ji-Hai": "excellent", "Xin-Hai": "inauspicious", "Gui-Hai": "dire" },
    ...
    Zi: {
      officer: "Bi",
      pillars: { "Jia-Zi": "fair", "Bing-Zi": "inauspicious", "Wu-Zi": "fair", "Geng-Zi": "auspicious", "Ren-Zi": "inauspicious" },
      good_for: ["minor_activities"],
 ...
 */
export function getDongGongRating(
  month: number,
  dayBranch: string,
  stem: string,
): DongGongRating | null {
  const monthData = DONG_GONG[month];
  if (!monthData) return null;
  const dayData = monthData[dayBranch] as DayBranchEntry | undefined;
  if (!dayData || !('pillars' in dayData)) return null;
  const pillarKey = `${stem}-${dayBranch}`;
  return (dayData.pillars[pillarKey] as DongGongRating) ?? null;
}

/**
 * Get complete day information including officer, ratings, and descriptions.
 */
export function getDongGongDayInfo(
  month: number,
  dayBranch: string,
): DayBranchEntry | null {
  const monthData = DONG_GONG[month];
  if (!monthData) return null;
  const dayData = monthData[dayBranch];
  if (!dayData || !('pillars' in dayData)) return null;
  return dayData as DayBranchEntry;
}

// Keywords that indicate positive/conditional use in an inauspicious day description
const CONSULT_POSITIVE_KEYWORDS = [
 ...
 * description tone, return consult promotion info.
 */
export function checkConsultPromotion(
  rating: string,
  dayInfo: DayBranchEntry | null,
): { promoted: boolean; reason: string } | null {
  if (rating !== "inauspicious" || !dayInfo) return null;

  const goodFor = dayInfo.good_for ?? [];
  const desc = (dayInfo.description_english ?? "").toLowerCase();
  const hasPositiveDesc = CONSULT_POSITIVE_KEYWORDS.some((kw) => desc.includes(kw));

  if (goodFor.length > 0 && hasPositiveDesc) {
    const activities = goodFor.map((a) => a.replace(/_/g, " ")).join(", ");
    return { promoted: true, reason: `Good for: ${activities}` };
  }
