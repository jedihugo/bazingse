...
import { STEMS, type StemName } from '../core';
import { getTenGod } from '../derived';
...
function _getLevel(score: number): typeof SENSITIVITY_LEVELS[number] {
  /** Get the sensitivity level for a given score. */
  const clamped = Math.max(0, Math.min(100, score));
  for (const level of SENSITIVITY_LEVELS) {
    if (level.min <= clamped && clamped <= level.max) {
      return level;
    }
  }
...
function _generateGuidance(
  score: number,
  _indicators: Array<Record<string, unknown>>,
  presentShenSha: Record<string, ShenShaResult>,
): [string, string] {
  ...
  if (score <= 20) {
    ...
    let en = "You have real spiritual sensitivity. Consider structured spiritual practice — meditation, qigong, or temple visits.";
    let zh = "你有真正的灵性敏感度。建议进行有结构的灵性修行——冥想、气功或寺庙参拜。";
    if ("华盖" in presentShenSha) {
    ...
    let en = "Strong spiritual sensitivity — you likely already experience unexplainable phenomena. This ability should be DEVELOPED, not suppressed. Suppressing it causes anxiety, insomnia, and restlessness.";
    let zh = "灵性敏感度很强——你很可能已经经历过无法解释的现象。这种能力应该被发展，而不是压制。压制它会导致焦虑、失眠和不安。";
    if ("童子" in presentShenSha) {
    ...
    let en = "Extremely high spiritual sensitivity — your 'third eye' is naturally open. You likely see, sense, or dream things others cannot. This is NOT a disorder — it is a genuine ability that needs proper management.";
    let zh = '极高的灵性敏感度——你的"天眼"自然开启。你很可能看到、感知到或梦到别人无法触及的事物。这不是病——这是真正的能力，需要正确管理。';
    ...
    if ("华盖" in presentShenSha && "空亡" in presentShenSha) {
...
export function assessSpiritualSensitivity(
  chart: ChartData,
  shenShaResults: ShenShaResult[],
  qiPhaseSpiritualBonus: number = 0,
  ...
  const presentShenSha: Record<string, ShenShaResult> = {};
  for (const r of shenShaResults) {
    if (r.present) {
      presentShenSha[r.name_chinese] = r;
    }
  }
  ...
  for (const r of shenShaResults) {
    if (r.name_chinese === "空亡" && r.present && (r.palace === "day" || r.palace === "hour")) {
      const palaceZh: Record<string, string> = { day: "日柱", hour: "时柱" };
      const w = 15;
      score += w;
      indicators.push({
        name: `Void (Kong Wang) in ${r.palace} pillar`,
        name_zh: `空亡在${palaceZh[r.palace!]}`,
        weight: w,
        reason: `Empty space in ${r.palace} palace = thin barrier between worlds`,
      });
  ...
  for (const pos of ["year", "month", "day", "hour"]) {
    const stem = chart.pillars[pos].stem;
    if (stem !== dm) {
  ...
  if (irCount >= 2) {
    ...
    indicators.push({
      ...
      name_zh: "偏印突出",
      weight: w,
      reason: `Unconventional perception — ${irCount} Indirect Resource stems found. Channel for non-physical knowledge`,
    });
  } else if (irCount === 1) {
    const w = 8;
    score += w;
  ...
  if (dm === "Ren" || dm === "Gui") {
    ...
    score += w;
    indicators.push({
      name: `Water Day Master (${STEMS[dm].chinese})`,
      name_zh: `日主${STEMS[dm].chinese}水`,
      weight: w,
  ...
  if (yinCount >= 3) {
    ...
    score += w;
    indicators.push({
      name: `Multiple Yin Stems (${yinCount}/4)`,
      name_zh: `多阴干(${yinCount}/4)`,
      weight: w,
  ...
  if (qiPhaseSpiritualBonus > 0) {
    score += qiPhaseSpiritualBonus;
    indicators.push({
      name: "Qi Phase + Shen Sha Tandem Effects",
      name_zh: "十二长生+神煞联动效应",
      weight: qiPhaseSpiritualBonus,
  ...
  // Get level
  const levelInfo = _getLevel(score);

  // Generate guidance
  const [guidanceEn, guidanceZh] = _generateGuidance(score, indicators, presentShenSha);

  return {
    score,
    level: levelInfo.level,
    label_en: levelInfo.label_en,
    label_zh: levelInfo.label_zh,
    description_en: levelInfo.description_en,
    description_zh: levelInfo.description_zh,
    indicators,
